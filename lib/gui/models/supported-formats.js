...
SupportedFormats.service('SupportedFormatsModel', function() {
  ...
  /**
   * @summary Check if an image seems a Windows image
   * @function
   * @public
   *
   * @param {String} imagePath - image path
   ...
   *
   * @example
   * if (SupportedFormatsModel.seemsWindowsImage('path/to/en_windows_7_ultimate_with_sp1_x86_dvd_u_677460.iso')) {
   *   console.log('Looks like a Windows image');
   * }
   */
  this.seemsWindowsImage = (imagePath) => {
    const re = /^.+_windows_.+(?:x86|x64|ia64|arm)_dvd_.+$/;
    return re.test(path.basename(imagePath));
  };
